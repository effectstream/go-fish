 ...
 *
 * With the in-browser wallet, batcher mode is the default — users no longer
 * need the Lace extension. Set VITE_BATCHER_MODE_ENABLED=false to explicitly
...
/**
 * Initialize providers for batcher mode with in-browser wallet.
 * Each user gets their own persistent identity (ZswapSecretKeys stored in localStorage)
 * while all transactions are delegated to the batcher for dust balancing and submission.
 */
export async function initializeBatcherProviders(): Promise<GoFishProviders> {
  ...
  const encryptionPublicKey = getLocalEncryptionPublicKey();
  console.log("[BatcherProviders] Using in-browser wallet identity");
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      ...
      accountId: String(coinPublicKey),
    }),
