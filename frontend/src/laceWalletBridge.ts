...
/**
 * Check if Lace wallet extension is connected (not the in-browser wallet).
 */
...
/**
 * Check if any wallet is available (Lace or in-browser).
 * Always returns true since the in-browser wallet is auto-created.
 */
...
/**
 * Get wallet address — returns Lace address if connected, otherwise in-browser wallet address.
 */
export function getLaceAddress(): string | null {
  return laceWalletAddress || getLocalWalletAddress();
}

/**
 * Get the full (non-truncated) wallet address for identity purposes.
 */
export function getWalletAddressFull(): string | null {
  return laceWalletAddress || getLocalWalletAddressFull();
}
...
export async function requestFaucetFunds(): Promise<{
  ...
  try {
    ...
    const url = `${API_BASE_URL}/api/faucet/nights?address=${encodeURIComponent(laceWalletAddress)}`;
